...
CHECK_INTERVAL = 120000  # 120 seconds
...
def get_gmail_messages():
    ...
    results = service.users().messages().list(userId='me', maxResults=100).execute()  # Fetch more to ensure we get the latest 10
    ...
    for message in messages:
        ...
        for header in headers:
            if header['name'] == 'Date':
                ...
                date = date.astimezone(timezone.utc)  # Convert to UTC
            if header['name'] == 'From':
                sender = header['value']
        if date and sender:
            snippet = msg['snippet']
    ...
    # Return only the latest 10 messages
...
def main():
    ...
    def insert_message(sender, date, message):
        ...
        analysis_results[message_counter] = (message, None)  # Save message for analysis
        message_counter += 1
    ...
    def display_messages():
        global message_counter
        message_counter = 1  # Reset the message counter for each fetch
        left_text.configure(state='normal')
        left_text.delete(1.0, tk.END)  # Clear the left text widget
